...
MAX_ROW_ERRORS = 50
MAX_FILE_SIZE_BYTES = 2 * 1024 * 1024  # 2MB for base64 payload


def validate_csv_headers(file_bytes: bytes) -> Optional[str]:
    """Validate CSV has required columns. Returns error message if invalid. Used for fail-fast in request."""
    try:
        text = file_bytes.decode("utf-8")
    except UnicodeDecodeError:
        return "File must be UTF-8 encoded."
    reader = csv.DictReader(io.StringIO(text))
    return _validate_csv_schema(reader)
...
async def process_import_file(
    ...
    """
    Process CSV import for an existing session. Used by async worker.
    Updates session in place. Raises HTTPException on validation errors.
    """
    account = await verify_account_ownership(db, session.user_id, session.account_id)
